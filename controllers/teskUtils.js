...
export const getLeastOccupiedPerson = async (size) => {
  ...
  for (let i = 0; i < all_persons.length; i++) {
    ...
    let personWithCapacity = await getPersonCapacityById(all_persons[i]._id);
...
export const setTaskAssignee = async (new_task) => {
  for (let i = 0; i < 2; i++) {
    let chosenPersonOccupation = await getLeastOccupiedPerson(new_task.size);
    let person = null;
    if (chosenPersonOccupation) {
      person = await getPersonCapacityById(chosenPersonOccupation.personId);
    }
